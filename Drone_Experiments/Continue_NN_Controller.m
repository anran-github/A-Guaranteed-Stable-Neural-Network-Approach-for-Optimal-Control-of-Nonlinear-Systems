...
global deltaT drone_mode Cont GoalPt DESIRED1 Kx Ky Kz x_init INIT_POS_ADJUST
...
%% LOAD NN MODELS
% Load ONNX model--X
netx = importNetworkFromONNX("weights_NOM/NN_x.onnx");
% netx = importNetworkFromONNX("weights_YALMIP/NN_x.onnx");
...
% Load ONNX model--Y
nety = importNetworkFromONNX("weights_NOM/NN_x.onnx");
% model = importONNXNetwork('model.onnx');
...
% Load ONNX model--X
netz = importNetworkFromONNX("weights_NOM/NN_z_1.onnx");
% model = importONNXNetwork('model.onnx');
...
global p state  inc FLAG SS CI Error ...
    Kx Ky Kz x_init INIT_POS_ADJUST...
...
if inc==1
    example_x = dlarray([state(1),0,0], 'UU');
else
...
if inc==1
    example_y = dlarray([state(2),0,0], 'UU');
else
...
if inc==1
    example_z = dlarray([state(3),0,1.5], 'UU');
else
