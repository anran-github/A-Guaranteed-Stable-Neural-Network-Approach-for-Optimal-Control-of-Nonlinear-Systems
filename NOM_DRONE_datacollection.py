...
x_r = torch.tensor([reference,0.]).reshape(2,1).to(device)
...
def constrain1(x):
    ...
    return torch.sum(c*torch.relu(1e-10-eig_values),dim=1).unsqueeze(-1)
...
def constrain2(args):
    ...
    # Add[:,1,0] = 0.1*9.8*torch.cos(args[:,0])
    Bdd = torch.tile(Bd, (x.shape[0], 1, 1))
...
def constrain3(args):
    ...
    # Add[:,1,0] = 0.1*9.8*torch.cos(args[:,0])
    Bdd = torch.tile(Bd, (x.shape[0], 1, 1))
    # x_rr optional: in pytorch, x_r boardcast processes automatically.
    # x_rr = torch.tile(x_r,(x.shape[0], 1, 1))


    res =  (1.5*theta)**2*torch.matmul(x_prime,x-x_r) - x_prime @ P @ (x-x_r) + 1e-10
    
    return c*torch.relu(res.squeeze(-1))
    # return c*relu_plus(res.squeeze(-1))
...
def constrain4(args):
    ...
    res = -(0.5*theta)**2*torch.matmul(x_prime,x-x_r) + x_dag@P@ (Add@x+Bdd@u-x_r) + 1e-10
...
if __name__ == '__main__':
    ...
    print('===========================')
    print(f'On Dirction of: {direction}')
    print(f"Input Theta: {args.theta}")
    print(f"Input Reference: {reference}")
    print('===========================')
    ...
    dataset_path = f'Drone_NOM_Dataset_{args.theta}_{direction}.txt'
    topk = 15


    # generate dataset states and references:
    if direction != 2:
        r,dataset_x1,dataset_x2 = np.meshgrid(reference,
                                            np.linspace(-1.,1.,101),
                                            np.linspace(-1,1,101))
        dataset_base = np.column_stack((dataset_x1.ravel(), dataset_x2.ravel(),r.ravel()))
    else:
        # change for z-direction
        r,dataset_x1,dataset_x2 = np.meshgrid(reference,
                                            np.linspace(0.5,2.5,101),
                                            np.linspace(-1,1,101))
        dataset_base = np.column_stack((dataset_x1.ravel(), dataset_x2.ravel(),r.ravel()))
    ...
    if  args.resume == True:
        ...
        querry = np.stack([data_temp[-1][0][0],data_temp[-1][0][1],reference]).round(4).reshape(1,3)
